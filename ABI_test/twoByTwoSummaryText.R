twoByTwoSummaryText_f = function(twoByTwoTables = NULL, whichText = NULL, categoryThreshold = 0.95){
  totLocs = nrow(subset(twoByTwoTables, waterClass == 1))

  pasteText = "error - choose algo"
  if(whichText == 1) {
    pasteText = paste0(round(100 * nrow(subset(twoByTwoTables, waterClass == 1 & Current_Value < categoryThreshold)) / totLocs, 0),
  ...
  if(whichText == 2) {
    pasteText = paste0(round(100 * nrow(subset(twoByTwoTables, waterClass == 3 & Current_Value < categoryThreshold)) / totLocs, 0),
